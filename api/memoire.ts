...
if (supabaseUrl && supabaseServiceKey) {
  try {
    ...
    console.warn('Failed to init Supabase Admin in api/memoire:', err);
  }
}
...
if (supabaseUrl && supabaseKey) {
  try {
    ...
    console.warn('Failed to init Supabase in api/memoire:', err);
  }
}
  ...
  try {
    ...
    if (req.method === 'POST') {
      const item = {
        ...
        contenu: req.body?.contenu || req.body?.texte || '',
        tags: Array.isArray(req.body?.tags) ? req.body.tags : ['général'],
        importance: typeof req.body?.importance === 'number' ? req.body.importance : 3,
    ...
    console.error('Erreur /api/memoire:', err);

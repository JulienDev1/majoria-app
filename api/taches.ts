...
if (supabaseUrl && supabaseServiceKey) {
  try {
    ...
    console.warn('Failed to init Supabase Admin in api/taches:', err);
  }
}
...
if (supabaseUrl && supabaseKey) {
  try {
    ...
    console.warn('Failed to init Supabase in api/taches:', err);
  }
}
  ...
  try {
    ...
    if (req.method === 'POST') {
      const item = {
        ...
        titre: req.body?.titre || req.body?.nom || 'Nouvelle tâche',
        description: req.body?.description || '',
        priorite: req.body?.priorite || 'normale',
        status: req.body?.status || 'attente',
        echeance: req.body?.echeance || null,
        user_id: userId,
        userId: userId,
        date_creation: req.body?.dateCreation || req.body?.date_creation || new Date().toISOString(),
